---
title: Configurer un déploiement système
---
## Contexte

Je voulais créer un processus de développement pour mon projet de documentation/site web `memo.apescasio.fr`. J’avais besoin d’un moyen de travailler en toute sécurité sur mon PC local tout en déployant rapidement les changements sur mon VPS. Les anciennes méthodes étaient : modifier manuellement les fichiers dans le VPS ou utiliser FTP pour envoyer des fichiers dans le VPS (chronophage et globalement une mauvaise idée). J’oubliais souvent quels fichiers j’avais modifiés ou écrasais accidentellement des mises à jour récentes.
...
Vu que j'utilise 'Fumadocs' comme template app, je dois avoir du nodejs + npm et pm2 pour que le site web memo.apescasio.fr soit disponible H24.
```bash title="~/memo.apescasio.fr.git/hooks/post-receive"
