...
<Step>
  ...
  Create the script to install and configure Active Directory Domain Services (AD DS), then
  schedule its execution after a reboot *(~5 seconds)*.
</Step>

<Step>
  ### Configuring DNS and DHCP (`3dns.ps1`) ### 
  Create the script to configure the DNS zone and install the DHCP service, then schedule its execution after a
  reboot *(~5 seconds)*.
</Step>
