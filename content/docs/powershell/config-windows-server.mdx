...
Dans presque tous les cours concernant le Windows serveur à l’IPI, j'ai dû réinstaller un serveur Windows avec AD DS, DNS, DHCP à la main dans VMWare. J'ai donc décidé de créer ce script qui automatisera l'ensemble du processus avec des configurations IP préconfiguré.
