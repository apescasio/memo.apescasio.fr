...
<Step>
  ### Configuration réseau ### Configurer l'adresse IP statique, le masque de
  sous-réseau et la passerelle par défaut, l'adresse du serveur DNS avec des
  valeurs prédéfinies *(~3 secondes)*.
</Step>

<Step>
  ### Activation de l'Autologon ### Configurer l'Autologon pour que
  l'utilisateur se connecte automatiquement après un redémarrage, en
  sauvegardant son mot de passe dans le registre *(~3 secondes)*.
</Step>

<Step>
  ### Création du dossier de scripts ### Créer un dossier pour stocker les
  scripts supplémentaires nécessaires à la configuration *(~5 secondes)*.
</Step>

<Step>
  ### Création et exécution du premier script (`2adds.ps1`) ### Créer le script
  pour installer et configurer Active Directory Domain Services (AD DS), puis
  planifier son exécution après redémarrage *(~5 secondes)*.
</Step>

<Step>
  ### Configuration DNS et DHCP (`3dns.ps1`) ### Créer le script pour configurer
  la zone DNS et installer le service DHCP, puis planifier son exécution après
  redémarrage *(~5 secondes)*.
</Step>

<Step>
  ### Redémarrage du serveur ### Redémarrer le serveur pour appliquer les
  changements, tout en assurant que les tâches planifiées reprennent après le
  reboot *(~180 secondes)*.
</Step>
...
<Callout type="warn">
  Ce script est à exécuter une fois que la machine virtuelle Windows Server est
