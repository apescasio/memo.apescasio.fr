...
Au début de ma deuxième alternance, je me suis rapidement rendu compte que certaines tâches revenaient fréquemment, notamment la rédaction des mêmes phrases pour clôturer des "tickets". 
...
Création d'une liste contenant des phrases fréquemment utilisées dans les tickets. Les phrases sont bilingues (français / anglais) pour permettre une réutilisation rapide selon le contexte *(~4 secondes)*.
</Step>
...
Avant d'exécuter le script, j'ai bien activé le presse-papier Windows en tapant sur la touche "Windows + V". 

![Activation de presse papiers Windows](/images/powershell/clipboard-enable.png)

Une fois le script exécuté, les phrases apparaissent correctement dans le presse-papiers. Malheureusement, selon Microsoft, il n'est pas possible d'automatiser pour épinger les phrases. Il faut donc les épingler manuellement, une par une.
